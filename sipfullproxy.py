...
rx_invalid = re.compile("^190\.168") #Edited
rx_invalid2 = re.compile("^12\.") #Edited
...
class UDPHandler(SocketServer.BaseRequestHandler):
	...
	def addTopVia(self):
		...
		for line in self.data:
			if rx_via.search(line) or rx_cvia.search(line):
				...
				if md:
					branch=md.group(1)
					via = "%s;branch=%sm" % (self.topvia, branch)
					data.append(via)
				# rport processing
	...
	def removeTopVia(self):
		...
		for line in self.data:
			if rx_via.search(line) or rx_cvia.search(line):
				if not line.startswith(self.topvia):
					data.append(line)
			else:
				data.append(line)
		return data
	...
	def processRegister(self):
		...
		if rx_invalid.search(contact) or rx_invalid2.search(contact):
			if self.registrar.has_key(fromm):
	...
	def processInvite(self):
		...
		if len(origin) == 0 or not self.registrar.has_key(origin):
			self.sendResponse("400 Bad Request")
			return
		destination = self.getDestination()
		if len(destination) > 0:
			if self.registrar.has_key(destination) and self.checkValidity(destination):
				socket,claddr = self.getSocketInfo(destination)
				#self.changeRequestUri()
				self.data = self.addTopVia()
				data = self.removeRouteHeader()
				#insert Record-Route
				data.insert(1,self.recordroute)
				text = string.join(data,"\r\n")
				socket.sendto(text , claddr)
				self.logger("Pouzivatel %s vola %s" % (origin, destination))
			else:
	...
	def processAck(self):
		destination = self.getDestination()
		if len(destination) > 0:
			if self.registrar.has_key(destination):
				socket,claddr = self.getSocketInfo(destination)
				#self.changeRequestUri()
				self.data = self.addTopVia()
				data = self.removeRouteHeader()
	...
	def processNonInvite(self):
		...
		if len(destination) > 0:
			if self.registrar.has_key(destination) and self.checkValidity(destination):
				...
				text = string.join(data,"\r\n")
				socket.sendto(text , claddr)
				self.logger("Hovor ukoncil %s" % origin)
			else:
	...
	def processCode(self):
		origin = self.getOrigin()
		if len(origin) > 0:
			if self.registrar.has_key(origin):
				socket,claddr = self.getSocketInfo(origin)
				self.data = self.removeRouteHeader()
				data = self.removeTopVia()
				text = string.join(data,"\r\n")
				#Edit 486
				text = text.replace("486 Busy Here", "486 Obsadene")
				text = text.replace("603 Decline", "486 Obsadene")
				socket.sendto(text,claddr)
				if text.find("486 Obsadene") != -1:
					self.logger("Hovor neprijaty")

	def logger(self, msg):
		if self.enable_logging:
			with open("proxy_log.txt", "a") as subor:
				current_time = time.strftime("%a, %d %b %Y %H:%M:%S ", time.localtime())
				subor.write(current_time + ": " + msg + "\n")

	def processRequest(self):
		#print "processRequest"
		if len(self.data) > 0:
			request_uri = self.data[0]
			#self.logger(request_uri)
